...
1. Length: You MUST write an exceptionally long, deep, and detailed lesson in the "content" field (minimum 1500 words). Expand heavily on every point. Do NOT write a brief summary.
2. Structure: You MUST use Markdown formatting with exactly these 4 headings: 
   ## Introduction
   ## Core Concepts
   ## Worked Examples
   ## Key Takeaways
3. Detail: Each section MUST have at least 4-5 extremely detailed paragraphs and comprehensive bullet points.
4. Examples: You MUST include at least 4 fully worked examples with step-by-step code, analogies, or solutions.
...
2. Structure: You MUST use Markdown formatting with exactly these 4 headings:
   ...
   ## Summary
3. Detail: Each section MUST have at least 4-5 extremely detailed paragraphs and comprehensive bullet points.
4. Examples: You MUST include at least 4 concrete examples or in-depth case studies.
5. Completeness: Do NOT stop early. Do NOT summarize. Write out the full lesson text explicitly.
...
async def generate_lesson(pace: str, topic: str, score: float, weak_areas: str, model_override: str | None = None) -> dict:
    ...
    t0 = time.time()
    raw = await _call_ollama(prompt, model=model, num_predict=6000, force_json=True)
